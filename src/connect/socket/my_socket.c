...
int _socket_client_connect(int sockfd, struct sockaddr *serv_addr, int timeout)
{
	...
	fd_set set;
	len = sizeof(int);
	printf("plc模块 使用客户端连接服务器  sockfd=%d", sockfd);
	if (sockfd < 0 || timeout < 0)
		return -1;
	ioctl(sockfd, FIONBIO, &ul); /*  设置为非阻塞模式*/
	printf("plc模块 设置为非阻塞模式");
	if (connect(sockfd, /*(struct sockaddr *)&*/ serv_addr, sizeof(struct sockaddr_in)) == -1)
	{
		...
		FD_ZERO(&set);
		FD_SET(sockfd, &set);
		printf("plc模块 等待  等待 select"); // comport
		if (select(sockfd + 1, NULL, &set, NULL, &tm) > 0)
		{
			...
			{
				close(sockfd);
				printf("plc模块 对方拒绝  %s:%d socket connect fail error:%s! \n", __FILE__, __LINE__, strerror(error));
				if (error != ETIMEDOUT)
				{
	...
	{
		close(sockfd);
		printf("plc模块 连接套接字失败！！！！！！！！！！！\r\n");
		ret = 0;
	}
	ul = 0;
	printf("plc模块 查看连接结果   ret=%d\r\n", ret);
...
int _socket_client_init(_SERVER_SOCKET *sock)
{
	...
	if (sock == NULL)
	{
		printf("plc模块 socket 结构为空！！！！");
		return -1;
	}

	printf("plc模块 建立Socket第一步 获取套接字");
	iType = (sock->protocol == UDP) ? SOCK_DGRAM : SOCK_STREAM;
	printf("plc模块 建立Socket第一步 获取套接字：选择TCU或UDP  iType=%d\r\n", iType);
	/*   一、获取套接字 */
	...
	/* 二、准备通信地址 */
	printf("plc模块 建立Socket第二步 准备通信地址");
